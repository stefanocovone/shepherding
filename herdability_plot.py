...
axs[0].set_xlabel(r'$M$', fontsize=16)
axs[0].set_ylabel(r'$N$', fontsize=16)

# Add a white line to demarcate the boundary for chi_rule
contour1 = axs[0].contour(chi_rule, levels=[0.99], colors='white', linewidths=2)
axs[0].clabel(contour1, fmt='%1.1f', colors='white', fontsize=12)

# Extract the contour line data for chi_rule using allsegs
contour1_coords = np.vstack(contour1.allsegs[0])
...
axs[0].plot(x_contour1, m_rule * x_contour1 + q_rule, color='black', linestyle='--', linewidth=2,
            label=f'y = {m_rule:.2f}x + {q_rule:.2f}')
axs[0].legend()
...
axs[1].set_title(r'$\chi$ Learning')
...
axs[1].set_xlabel(r'$M$', fontsize=16)
axs[1].set_ylabel(r'$N$', fontsize=16)

# Add a white line to demarcate the boundary for chi_learning
contour2 = axs[1].contour(chi_learning, levels=[0.99], colors='white', linewidths=2)
axs[1].clabel(contour2, fmt='%1.1f', colors='white', fontsize=12)

# Extract the contour line data for chi_learning using allsegs
contour2_coords = np.vstack(contour2.allsegs[0])
...
axs[1].plot(x_contour2, m_learning * x_contour2 + q_learning, color='black', linestyle='--', linewidth=2,
            label=f'y = {m_learning:.2f}x + {q_learning:.2f}')
axs[1].legend()
